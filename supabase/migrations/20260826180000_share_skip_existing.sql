-- Skip recipients who already own or have a share on the chatbot (no re-notify).
...
begin
  ...
  select public.chatbot_instance_id(f.chatbot_id), f.chatbot_id, b.name, b.created_by
  into v_instance, v_chatbot, v_bot_name, v_created_by
  ...
  loop
    ...
    -- Already has access: creator, existing share, or org owner/admin
    if v_uid = v_created_by
      or exists (
        select 1 from public.chatbot_shares s
        where s.chatbot_id = v_chatbot and s.user_id = v_uid
      )
      or exists (
        select 1
        from public.instance_members m
