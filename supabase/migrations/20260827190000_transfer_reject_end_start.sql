-- Reject transferring onto an End step (conversation would finish immediately / stick).
  ...
  if v_start is not null then
    ...
    loop
      if lower(trim(coalesce(v_node->>'key', ''))) = lower(v_start) then
        ...
        if lower(trim(coalesce(v_node->>'type', ''))) = 'end' then
          raise exception 'Cannot start transfer on an End step';
        end if;
        exit;
      end if;
    end loop;
