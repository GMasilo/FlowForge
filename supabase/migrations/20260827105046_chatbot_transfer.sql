-- Chatbot-to-chatbot transfer step + public transfer RPC.

alter type public.flow_node_type add value if not exists 'transfer';

-- ---------------------------------------------------------------------------
-- Transfer an active public conversation to another chatbot in the same org.
-- Does not require the target to be publicly enabled (internal handoff).
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Leave agent handoff before switching bots.
  ...
  -- Required entry variables from target chatbot settings.transferEntry.requiredVariables
  ...
  -- Replace session variables: do not carry over the source chatbot's vars.
  -- Only what the client sends (explicit mappings / pass-all + target defaults) applies.
