...
export async function executeHttpConnection(payload: {
  ...
  if (connectionId) {
    body = {
      ...
      // Include secrets when already loaded (preview fallback for older APIs / RPC denials).
      // Public chat (sessionId) must not rely on this — PHP ignores body secrets for sessions.
...
/** Designer media stored at files/{instanceId}/{chatbotId}/media */
...
/**
 * End-user conversation file stored at
 * files/{instanceId}/{chatbotId}/conversations/{sessionId}_{nodeKey}{ext}
 */
...
export async function purgeChatbotFiles(payload: {
  ...
  await postJson('/file/purge-chatbot', {
    instance_id: payload.instanceId,
    chatbot_id: payload.chatbotId,
  }, { signal: payload.signal })
}
