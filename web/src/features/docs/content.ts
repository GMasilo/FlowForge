    ...
    body: [
      {
        paragraphs: [
          ...
          'Choose Slack, Jira Automation, or Custom service. Slack supports either a channel incoming webhook URL or a bot token with channel ID. Bot tokens use Authorization: Bearer automatically and need chat:write permission and channel access. Both modes support message templates. Jira REST API mode creates issues with POST or updates an issue with PUT, using your Atlassian email and API token through Basic authentication. Enter the issue endpoint and a JSON body with fields; the example button supplies a starting body. Jira destinations always use REST API Basic authentication; older subscriptions must supply an Atlassian email, API token and issue endpoint. Custom destinations support optional headers and a JSON body template. Leave the JSON template blank to send the full event.',
