  ...
  return (
    <div className="ff-page-enter">
      <header className="max-w-2xl">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-accent)]/80">Help</p>
      ...
      <section className="mt-12">
        <h2 className="text-lg font-semibold text-[var(--color-ink)]">Guided topics</h2>
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2">
          {HELP_TOPICS.map((topic) => (
            <li key={topic.title}>
              <Link
                to={topic.to}
                className="group flex h-full flex-col rounded-2xl border border-transparent px-1 py-1 transition hover:border-[var(--color-accent)]/20 hover:bg-[var(--color-surface)]/50"
              >
                <span className="font-[family-name:var(--font-display)] text-[15px] font-semibold text-[var(--color-ink)] group-hover:text-[var(--color-accent)]">
                  {topic.title}
                </span>
      ...
      <section className="mt-14 max-w-2xl border-t border-[var(--color-border)]/60 pt-10">
        <div className="flex items-start gap-3">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[var(--color-accent)]/$1 text-[var(--color-accent)]">
            <MessageSquareText className="h-5 w-5" />
          </span>
          <div>
            <h2 className="text-lg font-semibold text-[var(--color-ink)]">Tips when something fails</h2>
            <ul className="mt-3 list-disc space-y-1.5 pl-5 text-[15px] leading-relaxed text-[var(--color-ink)]">
