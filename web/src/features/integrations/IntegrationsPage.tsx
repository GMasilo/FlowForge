...
function statusBadge(status: IntegrationStatus) {
  if (status === 'connected') return <Badge className="bg-emerald-100 text-emerald-800">Connected</Badge>
...
export function IntegrationsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {picking ? (
        <Card className="space-y-4 border-teal-200/70 bg-teal-50/20">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-base font-semibold text-slate-800">Choose a provider</h2>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {INTEGRATION_CATALOG.map((c) => (
              <button
                ...
                className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left transition hover:border-teal-300 hover:shadow-sm"
              >
                <span
                  className={cn(
      ...
      {open ? (
        <Card className="space-y-3 border-teal-200/70 bg-teal-50/30">
          ...
          {item ? <p className="text-sm text-slate-600">{item.description}</p> : null}
      ...
      {integrations.isLoading ? (
        ...
        <Card className="border-dashed text-center">
          <Puzzle className="mx-auto h-8 w-8 text-slate-400" />
