...
function measureTextWidth(el: HTMLElement, text: string): number {
  ...
  return (
    Math.ceil(measureEl.getBoundingClientRect().width) + (text.length ? 1 : 0)
  );
}
...
function fitTextControl(
  ...
  const hardCap =
    opts.maxWidth != null && opts.maxWidth > 0
...
function lineFromSegCaret(
  lines: string[],
  caret: number,
): { lineIdx: number; lineCaret: number } {
...
export function TemplateField({
  ...
  useLayoutEffect(() => {
    const lastTextId = [...segments]
      .reverse()
      .find((s) => s.kind === "text")?.id;
  ...
  function commit(next: Segment[], focus?: { segId: string; caret: number }) {
    ...
    requestAnimationFrame(() => {
      const target = cleaned.find(
        (s) => s.id === focus.segId && s.kind === "text",
      );
      const textSeg =
        target?.kind === "text"
          ? target
      ...
      const caret =
        textSeg.id === focus.segId ? focus.caret : textSeg.text.length;
  ...
  function analyzeChipDraft(text: string, caret: number) {
    ...
    const inner =
      start >= 0
        ? before.slice(start + 2).replace(/\}\}[\s\S]*$/, "")
        : chipLabel(text);
    setQuery(inner.trim());
    setOpen(true);
    setEditSegId(null);
  ...
  function updateText(segId: string, text: string, caret: number) {
    ...
    if (/\{\{[\s\S]*?\}\}/.test(text)) {
      ...
      commit(
        next,
        lastText
          ? { segId: lastText.id, caret: lastText.text.length }
          : undefined,
      );
      setOpen(false);
      setTokenStart(null);
    ...
    const next = segments.map((s) =>
      s.id === segId && s.kind === "text" ? { ...s, text } : s,
    );
    if (lineCountChanged) {
  ...
  function commitChipEdit(draft?: string) {
    ...
    if (
      ...
      const next = segments.map((s) =>
        s.id === chipId ? { ...s, kind: "chip" as const, raw } : s,
      );
      const after = next[idx + 1]?.kind === "text" ? next[idx + 1] : null;
      commit(
        next,
        after && after.kind === "text"
          ? { segId: after.id, caret: 0 }
          : undefined,
      );
      return;
    }
  ...
  function insertChipAt(
    ...
    const beforeSeg: TextSeg = {
      id: seg.id,
      kind: "text",
      text: seg.text.slice(0, start),
    };
    const chipSeg: ChipSeg = { id: nextId("c"), kind: "chip", raw };
    const afterSeg: TextSeg = {
      id: nextId("t"),
      kind: "text",
      text: seg.text.slice(end),
    };
    const next = [
      ...segments.slice(0, idx),
      beforeSeg,
  ...
  function onKeyDown(
    ...
    if (
      e.key === "Delete" &&
      lineCaret === el.value.length &&
      lineEnd === el.value.length &&
    ...
    <div className={cn("relative min-w-0", className)}>
      <div
        ...
        onClick={() => {
          ...
          const el = inputRefs.current.get(
            lineRefKey(lastText.id, lines.length - 1),
          );
          el?.focus();
        }}
      >
        ...
        >
          {segments.map((seg, segIndex) => {
                  ...
                  <input
                    ...
                    onBlur={() => {
                      window.setTimeout(() => {
                        ...
                        if (editingChipIdRef.current === seg.id)
                          commitChipEdit();
                      }, 120);
                    }}
                  />
            ...
            return lines.map((line, lineIdx) => {
              ...
              const isCaretSlot =
                isOnlyLine && line.length === 0 && !isLastText;
              ...
              return (
                <span key={refKey} className="contents">
                  {lineIdx > 0 ? (
                    <span className="h-0 w-full basis-full" aria-hidden />
                  ) : null}
                  <input
                    ref={(el) => {
                      if (el) {
      ...
      {!hideHint ? (
        <p className="mt-1 text-[11px] leading-snug text-[var(--color-ink-muted)]">
          ...
          vars, steps, and functions. Expressions work inline e.g.{" "}
